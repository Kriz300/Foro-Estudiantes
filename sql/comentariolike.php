<?php
    ...
    if (pg_num_rows($result) == 0) {
        ...
        $cl = (int) $row["likes"] + 1;
        $sql = "UPDATE Comentarios SET Likes = $2  WHERE comentarios.id=$1";
        ...
        if ($row["estadol"] == 0) {
            ...
            $sql = "UPDATE Comentarios SET Likes = $2  WHERE comentarios.id=$1";
            pg_query_params($con, $sql, array($_POST["key"], $cl));
            $sql = "UPDATE Estado_like_comentario SET estadol = $3 WHERE Estado_like_comentario.Rut = $1 and Estado_like_comentario.ID_Post = $2;";
